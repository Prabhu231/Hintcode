...
import MaximumProductSubarrayTestCase from "../../Database/Schemas/MaxProdSubarraySchema";
...
const runTestsForMaxProductSubarray = async (cppFunction: string) => {
  try {
    await connectDB();

    // Fetch test cases from MongoDB
    const testCases = await MaximumProductSubarrayTestCase.find();
...
export default runTestsForMaxProductSubarray;
