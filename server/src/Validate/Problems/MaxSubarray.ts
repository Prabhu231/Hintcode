...
import MaximumSubarrayTestCase from "../../Database/Schemas/MaxSubarraySchema";
...
const runTestsForMaximumSubarray = async (cppFunction: string) => {
  try {
    await connectDB();

    // Fetch test cases from MongoDB
    const testCases = await MaximumSubarrayTestCase.find();
...
export default runTestsForMaximumSubarray;
