...
import MissingNumberTestCase from "../../Database/Schemas/MissingNumberSchema";
...
const runTestsForMissingNumber = async (cppFunction: string) => {
  try {
    await connectDB();

    // Fetch test cases from MongoDB
    const testCases = await MissingNumberTestCase.find();
...
export default runTestsForMissingNumber;
