...
import LongestSubstringTestCase from "../../Database/Schemas/LongestSubstringSchema";
...
const runTestsForLongestSubstring = async (cppFunction: string) => {
  try {
    await connectDB();

    // Fetch test cases from MongoDB
    const testCases = await LongestSubstringTestCase.find();
    ...
    const cppCode = `
      ...
      int main() {
          ...
          for (const string &testcase : testCases) {
              int result = longestSubstring(testcase);
              cout << result << endl;
          }
          return 0;
      }`;
...
export default runTestsForLongestSubstring;
